...
pub fn sprite_to_hash(sprite: []const u8, allocator: std.mem.Allocator) ![]u8 {
...
pub fn combine_hashes(hashes: []const []const u8, allocator: std.mem.Allocator) ![]u8 {
...
pub fn finalize_hash(combined_hash: []const u8, allocator: std.mem.Allocator) ![]u8 {
    // Simply apply SHA-256 to the combined hash
    return sprite_to_hash(combined_hash, allocator);
}
...
pub fn extract_entropy(sprites: []const []const u8, allocator: std.mem.Allocator) ![]u8 {
    ...
    for (sprites, 0..) |sprite, i| {
        sprite_hashes[i] = try sprite_to_hash(sprite, allocator);
    }
    ...
    // Combine the hashes
    const combined = try combine_hashes(const_hashes, allocator);
